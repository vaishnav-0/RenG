...
internal class OperationRegistry(
    ...
    suspend fun writeStore(key: EngineRawResourceKey, resource: EngineStoredRawResource) {
        ...
        // verify against, so it is refused outright rather than skipping verification: Rentile 0.7.0
 ...
 * not something this seam can do.
    ...
    private fun passesClassSpecificReadValidation(resourceClass: ResourceClass, stored: StoredRawResource): Boolean =
        when (resourceClass) {
            ...
            // at Rentile 0.7.0), so a bad record in one of those classes self-heals on the next access and
 ...
 * class's extension back off a member's locator recovers the base the two share. Rentile 0.7.0 admits
